		...
		<v-row no-gutters>
			<v-col>
				<v-card id="pdfContent" color="primary" elevation="0">
					...
					<template #subtitle>
						<span class="pb-0 text-subtitle-2 font-weight-regular"> Mortality Rate per month </span>
					</template>
					<template #text>
						<MonthlyMortalityRateChart class="w-100" :mortality-rate-per-month></MonthlyMortalityRateChart>
					</template>
				</v-card>
			</v-col>
		</v-row>
...
import MonthlyMortalityRateChart from "@/components/app/analytics/MonthlyMortalityRateChart.vue"
import { nextTick, reactive, ref } from "vue"

//

const mortalityRatePerMonth = reactive<Record<string, number>>({
	January: 8,
	February: 13,
	March: 17,
	April: 20,
...
const onClickPrintPDF = async () => {
	...
	pdf.save(`monthly-mortality-rate-chart.pdf`)

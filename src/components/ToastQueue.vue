    ...
    <div style="z-index: 9999; width: min(80dvw, 350px)">
        <Transition>
            <div
                ...
                :key="queued.id"
                :class="`
                    px-5 py-3 rounded elevation-1 bg-${queued.color} 
                    d-flex align-center justify-space-between
                    position-fixed right-0 mr-1 mt-16 ${props[`msg-class`]}
                `"
            >
                <slot name="text">
...
const props = defineProps<{ "msg-class"?: string, timeout?: number, closable?: boolean }>()

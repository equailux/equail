...
import type { FeedConfigCreateSchema } from "@/schemas/FeedConfigSchema"
import { FeedConfigCreateSchema as FeedConfigCreateFormSchema } from "@/schemas/FeedConfigSchema"
...
const emit = defineEmits<{
	submit: [
		values: FeedConfigCreateSchema,
		ctx: SubmissionContext<{ [K in keyof FeedConfigCreateSchema]?: unknown }>
	]
	error: [error: unknown]
}>()

const { handleSubmit, isSubmitting } = useForm({
	validationSchema: toTypedSchema(FeedConfigCreateFormSchema),
	initialValues: {
		servo1Ms: 0,
		servo2Ms: 0,
		stepperMs: 0,

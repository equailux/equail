...
const emit = defineEmits<{
	submit: [
		values: ThresholdCreateValues | ThresholdUpdateSchema,
		ctx: SubmissionContext<{ [K in keyof ThresholdCreateValues]?: unknown }>
	]
	error: [error: unknown]
}>()

const isUpdate = computed(() => !!props.threshold)
const thresholdOperators = ThresholdOp

const { handleSubmit, isSubmitting } = useForm({
	validationSchema: toTypedSchema(isUpdate.value ? ThresholdUpdateFormSchema : ThresholdCreateSchema),
	initialValues: {
		name: props.threshold?.name ?? "",
		icon: props.threshold?.icon ?? "mdi-alert-outline",
		message: props.threshold?.message ?? "",
		operator: props.threshold?.operator ?? "All",
		activated: props.threshold?.activated ?? true,
		sensorId: props.threshold?.sensorId ?? props.sensors[0]?.id,
	},
})
...
const onSubmit = handleSubmit(async (values, ctx) => {
	await Promise.resolve()
		.then(() => emit("submit", values, ctx as SubmissionContext<{ [K in keyof ThresholdCreateValues]?: unknown }>))

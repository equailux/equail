...
import { SensorCreateSchema, type SensorCreateSchema as SensorCreateValues } from "@/schemas/SensorSchema"
...
const emit = defineEmits<{
	submit: [
		values: SensorCreateValues,
		ctx: SubmissionContext<{ [K in keyof SensorCreateValues]?: unknown }>
	]
	error: [error: unknown]
}>()

const { handleSubmit, isSubmitting } = useForm({
	validationSchema: toTypedSchema(SensorCreateSchema),
	initialValues: {
		name: "",
		tag: "",
		interval: 1,
